...
from em_framework import (ParameterUncertainty, ModelStructureInterface, 
...
class WaasModel(ModelStructureInterface):
    ...
    uncertainties = [
                     ParameterUncertainty((1,30), "climate scenarios",  integer=True),
    ...
    outcomes = [Outcome("Flood damage (Milj. Euro)"),
                Outcome("Number of casualties"),
                Outcome("Costs"),
                Outcome("Timing")]
    ...
    def run_model(self, case):
        ...
        self.output["Flood damage (Milj. Euro)"] = np.sum(self.output["Flood damage (Milj. Euro)"])
        self.output["Number of casualties"] = np.sum(self.output["Number of casualties"])
        self.output["Timing"] = [entry[1] for entry in self.timing]
        
        while len(self.output["Timing"])<3:
            self.output["Timing"].append(100)
...
#         command = r'C:\Program Files (x86)\pcraster-4.0.2_x86-32\bin\pcrcalc -f %s\RAMImiSUI_final.mod -b %s\bindings.txt %d %d %s' % (self.working_directory,
